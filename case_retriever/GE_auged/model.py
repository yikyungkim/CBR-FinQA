...
class Bert_model(nn.Module):
    ...
    def forward(self, input_ids, attention_mask,cand_input_ids = None, cand_attention_mask= None,label = None):
        ...
        if cand_input_ids is not None:
            cand_outputs = self.bert(
                input_ids=cand_input_ids, 
                attention_mask=cand_attention_mask)
            cand_output = cand_outputs.last_hidden_state[:, 0, :]
            cand_output = self.linear(cand_output)
            # output = bert_output
            return origin_output, cand_output
