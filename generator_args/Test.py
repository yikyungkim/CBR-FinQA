...
test_data, test_examples, op_list, const_list = \
    read_examples_test(input_path=conf.test_file, tokenizer=tokenizer,
...
def generate_test():
    ...
    with torch.no_grad():
        for x in tqdm(data_iterator):
            ...
            logits = model(input_ids, input_mask, segment_ids, option_mask)
    ...
    output_nbest_file = os.path.join(ksave_dir_mode, "nbest_predictions.json")
    ...
    exe_acc, prog_acc, op_acc = evaluate_result(
        output_nbest_file, conf.test_file, output_eval_file, output_error_file, program_mode=conf.program_mode)
