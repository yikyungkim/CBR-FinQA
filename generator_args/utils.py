...
def read_examples_test(input_path, tokenizer, op_list, const_list, log_file):
    ...
    examples = []
    for entry in input_data:        
        examples.append(finqa_utils.read_mathqa_entry_test(entry, tokenizer))
        program = examples[-1].program
        # for tok in program:
        #     if 'const_' in tok and not (tok in const_list):
        #         const_list.append(tok)
...
def predict_arguments(pred_logits, feature, example, const_list):

    mask_indices = feature.mask_indices
    pred_token_ids=[]
    for i in mask_indices:
        mask_logit = pred_logits[i]
        mask_softmax = _compute_softmax(mask_logit)
        pred_token_id = np.argmax(mask_softmax)
    ...
    for token_id in pred_token_ids:
        if token_id < len(const_list):
            pred_args.append(const_list[token_id])
        elif token_id < (len(const_list) + conf.max_seq_length):
            number_id = token_id - len(const_list)
            pred_args.append(numbers[number_indices.index(number_id)])
        else:
            pred_args.append('wrong')

    return pred_args



def compute_predictions(all_examples, all_features, all_results, n_best_size, const_list):
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
            "NbestPrediction", "options answer program")

        nbest = []
        for pred in prelim_predictions:
            if len(nbest) >= n_best_size:
                break
            pred_args = predict_arguments(pred.logits, pred.feature, example, const_list)
            pred_prog = finqa_utils.fill_masked_program(pred.feature, pred_args)
            nbest.append(
                _NbestPrediction(
...
class DataLoader:
    ...
    def get_batch(self):
        ...
        for each_data in self.data[start_index: end_index]:
            ...
            batch_data["masked_program"].append(each_data.masked_program)
            batch_data["arguments"].append(each_data.arguments)
            batch_data["argument_ids"].append(each_data.argument_ids)
            batch_data["argument_mask"].append(each_data.argument_mask)
            batch_data["mask_indices"].append(each_data.mask_indices)
...
"""Added for Operator Accuracy"""
def equal_operator(gold, pred):
    ...
    while i < len(gold):
        if gold[i]!='EOF':
            ops_gold.append(gold[i])
        i+=4
    while j < len(pred):
        if pred[j]!='EOF':
            ops_pred.append(pred[j])
        j+=4
    
    if ops_gold==ops_pred:
        return True
    else:
        return False
...
def evaluate_result(json_in, json_ori, all_res_file, error_file, program_mode):
    ...
    op_acc = float(op_correct) / len(data)

    print("All: ", len(data))
    print("Correct: ", correct)
    print("Exe acc: ", exe_acc)
    print("Prog acc: ", prog_acc)
    print("Operator acc: ", op_acc)
    ...
    return exe_acc, prog_acc, op_acc
