...
def read_mathqa_entry(entry, case_entry, tokenizer, num_case, input_concat, program_type):
    ...
    if case_entry['model_input']:
        for case in case_entry['model_input'][:num_case]:
            ...
            if program_type == "prog":
                ...
                case_program = operators_in_program(case['program'][:-5])
            
            if input_concat == "qandp":
                case_pair = " " + tokenizer.sep_token + " " + case_question.strip() + " " + '[QNP]' + " " + case_program
            elif input_concat == "ponly":
                case_pair = " " + tokenizer.sep_token + " " + case_program
            case_pairs += case_pair
        original_question += case_pairs
