...
class MathQAExample(
        collections.namedtuple(
            ...
            numbers number_indices original_program program case_pairs case_tokens"
        )):
...
"""added for CBR"""
def get_case_inputs(example, max_seq_len, tokenizer, cls_token, sep_token):

    case_tokens = example.case_tokens

    # case_question = example.case_question
    # case_program = example.case_program
    # case_question_program = case_question + " " + '[QNP]' + " " + case_program
    # case_tokens = case_question_program.split(' ')
    tokens = [cls_token] + case_tokens + [sep_token]
    
    case_ids = tokenizer.convert_tokens_to_ids(tokens)
    case_mask = [1] * len(case_ids)
    case_segs = [0] * len(tokens)
    
    padding = [0] * (max_seq_len - len(case_ids))
    case_ids.extend(padding)
    case_mask.extend(padding)
    case_segs.extend(padding)

    assert len(case_ids) == max_seq_len
    assert len(case_mask) == max_seq_len
    assert len(case_segs) == max_seq_len
    
    return case_ids, case_mask, case_segs
...
def convert_single_mathqa_example(example, is_training, tokenizer, max_seq_length,
    ...
    for ind, offset in enumerate(example.number_indices):
        if offset < len(input_mask):                # input_mask :
            input_mask[offset] = 2                  # tokens = 1, numbers = 2, else = 0
        else:
            if is_training == True:
                # print("\n")
                # print("################")
    ...
    for ind in range(len(number_mask)):
        if number_mask[ind] < 0:
            number_mask[ind] = 0                    # number_mask: numbers 1, else 0


    """added for CBR"""
    case_ids, case_mask, case_segs = get_case_inputs(example, max_seq_length, tokenizer, cls_token, sep_token)    
    number_case = [0] * len(case_mask)

    option_mask = [1, 0, 0, 1] + [1] * (len(op_list) + len(const_list) - 4)
    option_mask = option_mask + number_mask + number_case
    option_mask = [float(tmp) for tmp in option_mask]       # option_mask: special tokens & numbers 1, else 0

    for ind in range(len(input_mask)):
        if input_mask[ind] > 1:
            input_mask[ind] = 1                    # drop 2 in input_mask
...
def read_mathqa_entry(entry, case_entry, tokenizer):
    ...
    """ added for CBR """
    case_pairs=""
    if case_entry['model_input']:
        for case in case_entry['model_input']:
            case_question = case['question']
            case_program = case['program'][:-5]
            case_pair = " " + '[CNC]' + " " + case_question.strip() + " " + '[QNP]' + " " + case_program.strip()
            case_pairs += case_pair
    
    org_case_tokens = case_pairs.split(' ')
    case_tokens=[]
    for i, tok in enumerate(org_case_tokens):
        tok_proc = tokenize(tokenizer, tok)
        case_tokens.extend(tok_proc)
    
    # if case_entry['model_input']!=[]:
    #     case = case_entry['model_input'][0]
    #     case_question = case['question']
    #     case_program = case['program'][:-5]
    # else:
    #     case_question = ""
