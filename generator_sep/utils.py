...
def read_examples(input_path, case_path, tokenizer, op_list, const_list, log_file):
    ...
    for entry, case_entry in zip(input_data, case_data):
        examples.append(finqa_utils.read_mathqa_entry(entry, case_entry, tokenizer))
...
def read_examples_test(input_path, case_path, tokenizer, op_list, const_list, log_file, threshold):
    ...
    for entry, case_entry in zip(input_data, case_data):
        if case_entry['model_input']!=[]:
            if case_entry['model_input'][0]['score'] >= threshold:
                examples.append(finqa_utils.read_mathqa_entry(entry, case_entry, tokenizer))
            else:
                continue
...
class DataLoader:
    ...
    def get_batch(self):
        ...
        for each_data in self.data[start_index: end_index]:
            ...
            """added for CBR"""
            batch_data["case_ids"].append(each_data.case_ids)
            batch_data["case_mask"].append(each_data.case_mask)
            batch_data["case_segs"].append(each_data.case_segs)
...
def evaluate_result(json_in, json_ori, case_file, all_res_file, error_file, program_mode):
    '''
    execution acc
    program acc
    '''
    ...
    with open(case_file) as f_in:
        data_case = json.load(f_in)
    ...
    for tmp, tmp2 in zip(data, data_case):
        ...
        each_ori_data["qa"]["case_input"] = case_input
