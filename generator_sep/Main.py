...
if conf.mode == "train":
    ...
    os.makedirs(saved_model_path, exist_ok=True)       # resume
...
def train():
    ...
    for ep in range(ep_global, conf.epoch):                     
        ...
        for x in train_iterator:
            ...
            """added for CBR"""
            case_ids = torch.tensor(x['case_ids']).to(conf.device)
            case_mask = torch.tensor(x['case_mask']).to(conf.device)
            case_segs = torch.tensor(x['case_segs']).to(conf.device)

            model.zero_grad()
            optimizer.zero_grad()

            this_logits = model(True, input_ids, input_mask, segment_ids, case_ids, case_mask, case_segs,
            ...
            wandb.log({"loss/train_loss": this_loss.item(), "params/batch": k})
...
def evaluate(data_ori, data, model, ksave_dir, mode='valid'):
    ...
    if mode == "valid":
        ...
        case_file = conf.valid_case
    else:
        original_file = conf.test_file
        case_file = conf.test_case

    exe_acc, prog_acc, op_acc = evaluate_result(
        output_nbest_file, original_file, case_file, output_eval_file, output_error_file, program_mode=conf.program_mode)
...
if __name__ == '__main__':
    wandb.init(project="generator", entity="yikyung_project")
