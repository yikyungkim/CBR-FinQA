...
class Bert_model(nn.Module):
    ...
    def __init__(self, num_decoder_layers, hidden_size, dropout_rate, input_length,
        ...
        other_ones = nn.Parameter(torch.ones(
            input_length * 2 + self.const_list_size), requires_grad=False)
        ...
        para_after_ones = nn.Parameter(torch.ones(
            input_length * 2 + self.reserved_token_size - op_list.index(')') - 1), requires_grad=False)
        ...
        self.step_masks = nn.Parameter(torch.zeros(
            conf.max_step_ind, input_length * 2 + self.reserved_token_size), requires_grad=False)
        ...
        """ added for CBR """
        self.case_cls_prj = nn.Linear(hidden_size, hidden_size, bias=True)
        self.case_cls_dropout = nn.Dropout(dropout_rate)
        self.case_seq_prj = nn.Linear(hidden_size, hidden_size, bias=True)
        self.case_seq_dropout = nn.Dropout(dropout_rate)                
        ...
        """ added for CBR """
        self.case_attn_prj = nn.Linear(hidden_size, hidden_size, bias=True)
        self.case_attn_dropout = nn.Dropout(dropout_rate)
        ...
        self.decoder_step_proj = nn.Linear(
            4*hidden_size, hidden_size, bias=True)
        self.decoder_step_proj_dropout = nn.Dropout(dropout_rate)

        self.step_mix_proj = nn.Linear(
            hidden_size*2, hidden_size, bias=True)

    def forward(self, is_training, input_ids, input_mask, segment_ids, case_ids, case_mask, case_segs, option_mask, program_ids, program_mask, device):
        ...
        # print(self.program_length)                    # 30
        # print(program_ids.size())                     # [B 30]
        # print(split_program_ids[0].size())            # [1 1]
        ...
        sequence_output = self.seq_dropout(sequence_output)                          # [B 512 d]

        """added for CBR"""
        case_bert_outputs = self.bert(input_ids = case_ids, attention_mask=case_mask, token_type_ids=case_segs)
        case_bert_sequence_output = case_bert_outputs.last_hidden_state
        case_sequence_output = self.case_seq_prj(case_bert_sequence_output)
        case_sequence_output = self.case_seq_dropout(case_bert_sequence_output)     # [B 512 d]

        op_embeddings = self.reserved_token_embedding(self.reserved_ind)        # [44 d]
        op_embeddings = op_embeddings.repeat(batch_size, 1, 1)                  # [B 44 d]

        logits = []

        init_decoder_output = self.reserved_token_embedding(self.reserved_go)   # [1 d]
        decoder_output = init_decoder_output.repeat(batch_size, 1, 1)           # [B 1 d]

        # [batch, op + seq len, hidden]
        initial_option_embeddings = torch.cat(
            [op_embeddings, sequence_output, case_sequence_output], dim=1)      # [B 1068 d]
        ...
        decoder_state_h = torch.zeros(1, batch_size, self.hidden_size, device=device)   # [B 1 d]
        decoder_state_c = torch.zeros(1, batch_size, self.hidden_size, device=device)

        float_input_mask = input_mask.float()                           # [B 512]
        float_input_mask = torch.unsqueeze(float_input_mask, dim=-1)    # [B 512 1]

        """added for CBR"""             
        float_case_mask = case_mask.float()                             # [B 512]                            
        float_case_mask = torch.unsqueeze(float_case_mask, dim=-1)      # [B 512 1]
        ...
        for cur_step in range(self.program_length):
            ...
            decoder_history_attn_vec = self.decoder_history_attn_prj(decoder_output) 
            decoder_history_attn_vec = self.decoder_history_attn_dropout(decoder_history_attn_vec) # [B 1 d]

            decoder_history_attn_w = torch.matmul(decoder_history, torch.transpose(decoder_history_attn_vec, 1, 2)) # [B 1 d] x [B d 1] = [B 1 1]
            decoder_history_attn_w = F.softmax(decoder_history_attn_w, dim=1)

            decoder_history_ctx_embeddings = torch.matmul(torch.transpose(decoder_history_attn_w, 1, 2), decoder_history)  # [B 1 1] x [B 1 d] = [B 1 d]

            if conf.sep_attention:
                # input sequence attention
                question_attn_vec = self.question_attn_prj(decoder_output)
                question_attn_vec = self.question_attn_dropout(question_attn_vec)       # [B 1 d]

                question_attn_w = torch.matmul(sequence_output, torch.transpose(question_attn_vec, 1, 2))  # [B 512 d] x [B d 1] = [B 512 1]
                question_attn_w -= 1e6 * (1 - float_input_mask)
                question_attn_w = F.softmax(question_attn_w, dim=1)     

                question_ctx_embeddings = torch.matmul(torch.transpose(question_attn_w, 1, 2), sequence_output)    # [B 1 512] x [B 512 d] = [B 1 d]

            # another input sequence attention
            question_summary_vec = self.question_summary_attn_prj(decoder_output)
            question_summary_vec = self.question_summary_attn_dropout(question_summary_vec)
            ...
            """added for CBR"""
            # case sequence attention
            case_attn_vec = self.case_attn_prj(decoder_output)                  
            case_attn_vec = self.case_attn_dropout(case_attn_vec)               # [B 1 d]
            
            # attention score
            case_attn_w = torch.matmul(case_sequence_output, torch.transpose(case_attn_vec, 1, 2))  # [B 512 d] x [B d 1] = [B 512 1]
            case_attn_w -= 1e6 * (1 - float_case_mask)
            # attention coefficient
            case_attn_w = F.softmax(case_attn_w, dim=1)         
            # attention value
            case_embeddings = torch.matmul(torch.transpose(case_attn_w, 1, 2), case_sequence_output) # [B 1 512] x [B 512 d] = [B 1 d]
            ...
            if conf.sep_attention:
                concat_input_embeddings = torch.cat([decoder_history_ctx_embeddings,
                                                     ...
                                                     case_embeddings,
                                                     decoder_output], dim=-1)           # [B 1 4d]
            else:
                concat_input_embeddings = torch.cat([decoder_history_ctx_embeddings,
                                                     decoder_output], dim=-1)

            input_embeddings = self.input_embeddings_prj(concat_input_embeddings)       # [B 1 d]  (c)

            if conf.layer_norm:
                input_embeddings = self.input_embeddings_layernorm(input_embeddings)

            question_option_vec = this_step_new_op_emb * question_summary_embeddings    # [B 1068 d] * [B 1 d]  = [B 1068 d] (dot product)
            option_embeddings = torch.cat(
                [this_step_new_op_emb, question_option_vec], dim=-1)                    # [B 1068 2d]

            option_embeddings = self.option_embeddings_prj(option_embeddings)           # [B 1068 d] (H')

            option_logits = torch.matmul(option_embeddings, torch.transpose(input_embeddings, 1, 2))    # [B 1068 d] x [B d 1] = [B 1068 1]  (H' x c)
            option_logits = torch.squeeze(option_logits, dim=2)                         # [B, 1068]  (1068개의 vocab에 대한 classification)
            option_logits -= 1e6 * (1 - option_mask)                                    # option에 대한 logits (special tokens & numbers)
            ...
            if is_training:
                ...
                if cur_step % 4 == 0 or (cur_step + 1) % 4 == 0:
                    # op round
                    option_logits -= 1e6 * self.seq_only_mask       # ops 빼고는 다 0으로 만들기
                else:
                    # number round
                    option_logits -= 1e6 * self.op_only_mask        # others 빼고는 다 0으로 만들기
            ...
            if (cur_step + 1) % 4 == 0:
                ...
                decoder_step_vec = self.decoder_step_proj(
                    concat_input_embeddings)        # concatT_input_embeddings = C_T
                decoder_step_vec = self.decoder_step_proj_dropout( 
                    decoder_step_vec)   # B 1 H
                decoder_step_vec = torch.squeeze(decoder_step_vec)  # B
                ...
                this_step_new_emb = this_step_new_emb.repeat(
                    1, self.reserved_token_size+self.input_length*2, 1)  # [batch, op seq, hidden]
